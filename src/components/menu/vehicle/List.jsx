...
export default function List(params) {
  ...
  //   try {
  //     const groupIds = [...new Set(vehicles.map((v) => v.groupId))];
  
  //     if (groupIds.length === 0) return; // Avoid unnecessary requests
  ...
  const handleDragEnd = async (event) => {
    ...
    //  Persist the updated order to the backend
     try {
      await axios.post("http://localhost:8383/vehicle/update-order", { items: updatedVehicles });
    } catch (error) {
  ...
  const filteredVehicles = vehicles.filter((v) => {
    ...
    return ["name", "inspectionExpiry", "inspectionExpiry","status"].some((key) =>
      String(v[key] || "").toLowerCase().includes(query)
    ) ||
      ...
      <div className='container'>
        <section className='searchAndFilter'>
          ...
          <div className="filterContainer">
            ...
            <button className='createNewBtn' onClick={handleCreateModelBox}>+ Create New</button>
          </div>
        </section>
        ...
        <section className='list'>
          <div className='vehicleListHeader'>
            ...
            <div>
              <p>Group</p>
              <svg onClick={sortGroupName} xmlns="http://www.w3.org/2000/svg" width="16" viewBox="0 0 21 24" fill="currentColor">
                <path d="M10.4587 21L14.3809 14.5H6.5365L10.4587 21ZM10.4587 3L14.3809 9.5H6.5365L10.4587 3Z" fill="white" stroke="white" stroke-width="2" stroke-linejoin="round"/>
              </svg>
            </div>
            <div>
              <p>Inspection Expiry</p>
              <svg onClick={()=>sortVehicles('inspectionExpiry')} xmlns="http://www.w3.org/2000/svg" width="16" viewBox="0 0 21 24" fill="currentColor">
                <path d="M10.4587 21L14.3809 14.5H6.5365L10.4587 21ZM10.4587 3L14.3809 9.5H6.5365L10.4587 3Z" fill="white" stroke="white" stroke-width="2" stroke-linejoin="round"/>
              </svg>
            </div>
            <div>
              <p>Insurance Expiry</p>
              <svg onClick={() => sortVehicles('insuranceExpiry')} xmlns="http://www.w3.org/2000/svg" width="16" viewBox="0 0 21 24" fill="currentColor">
                <path d="M10.4587 21L14.3809 14.5H6.5365L10.4587 21ZM10.4587 3L14.3809 9.5H6.5365L10.4587 3Z" fill="white" stroke="white" stroke-width="2" stroke-linejoin="round"/>
              </svg>
            </div>
            <div>
              <p>Status</p>
              <svg onClick={() => sortVehicles('status')} xmlns="http://www.w3.org/2000/svg" width="16" viewBox="0 0 21 24" fill="currentColor">
                <path d="M10.4587 21L14.3809 14.5H6.5365L10.4587 21ZM10.4587 3L14.3809 9.5H6.5365L10.4587 3Z" fill="white" stroke="white" stroke-width="2" stroke-linejoin="round"/>
              </svg>
            </div>
            <div></div>
