...
export default function List(params) {
  ...
  const {
    ...
    loading,
    error,
  } = useFetchData("http://localhost:8383/vehicle/list", deleteStatus);
  ...
  const handleDragEnd = async (event) => {
    ...
    try {
      await axios.post("http://localhost:8383/vehicle/update-order", {
  ...
  const filteredVehicles = vehicles.filter((v) => {
    ...
    return (
      ["name", "inspectionExpiry", "inspectionExpiry", "status"].some((key) =>
        String(v[key] || "")
          .toLowerCase()
          .includes(query)
      ) ||
