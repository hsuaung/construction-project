...
export const Task = ({ id, name,color, onClick, onSuccess }) => {
  const { attributes, listeners, setNodeRef, transition, transform } = useSortable({ id });

  const [siteoperation,setSiteoperation] = useState([])
  const { data: siteOperations } = useFetchData( id ? `http://localhost:8383/siteoperation/getbyoperationtypeid/${id}` : null);
  useEffect(() => {
    if(siteOperations)
      setSiteoperation(siteOperations)      
  },[siteOperations])
  // console.log(siteoperation);

  const numberOfSites = siteOperations.length;
  const style = {
  ...
  return (
    <div ref={setNodeRef}  style={style} className="data">
      ...
      <svg xmlns="http://www.w3.org/2000/svg" width="20" viewBox="0 0 30 30" fill="none">
                     ...
                     <path d="M17.5 11.25C18.1904 11.25 18.75 10.6904 18.75 10C18.75 9.30964 18.1904 8.75 17.5 8.75C16.8096 8.75 16.25 9.30964 16.25 10C16.25 10.6904 16.8096 11.25 17.5 11.25Z" fill="#F27D14"/>                     <path d="M17.5 21.25C18.1904 21.25 18.75 20.6904 18.75 20C18.75 19.3096 18.1904 18.75 17.5 18.75C16.8096 18.75 16.25 19.3096 16.25 20C16.25 20.6904 16.8096 21.25 17.5 21.25Z" fill="#F27D14"/>
      ...
      <div style={{ backgroundColor: color, width: "15px", height: "15px", borderRadius: "10%" }}></div>
      <p>{name}</p>
      </div>
      <p>{numberOfSites}</p>
